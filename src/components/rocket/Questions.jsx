...
const ExternalLink = ({ href, children }) => (
  <a href={href} target="_blank" rel="noreferrer" className="underline text-Neutral-700 hover:text-Neutral-400">
    {children}
  </a>
);
...
function Listitem({ content }) {
  return content.map((item, index) => {
    return (
      <div
        className={` w-full max-w-83 flex gap-4  items-start justify-start  py-5  border-b-2 border-Neutral-200 md:max-w-2xl lg:max-w-214 lg:p-5 lg:gap-8 `}
        key={index}
      >
        <div className="list-number hidden lg:flex lg:items-start lg:justify-start">
          <img className="object-cover max-w-6 h-6" src={questionIconsLg[index]} alt={`number${index + 1}`} />
        </div>
        <div className="list-number lg:hidden flex items-center justify-center">
          <img  className="object-cover max-w-5 h-5" src={questionIconsSm[index]}  alt={`number${index + 1}`} />
        </div>
...
function Questions() {
  return (
    <section className="w-full bg-Neutral-white flex items-center justify-center">
      <div className="list w-full max-w-83 bg-Neutral-white flex flex-col items-center justify-center gap-12 md:max-w-2xl lg:max-w-214 lg:gap-15 ">
